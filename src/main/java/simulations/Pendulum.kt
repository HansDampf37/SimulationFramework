...
class Pendulum(private val amountOfPoints: Int) : Simulation("Pendulum") {
    ...
    private val gravity = Vec3BLablabla(0.0, -9.81, 0.0)
    private lateinit var points: Array<Point3d>
    private lateinit var velocities: Array<Vec3BLablabla>
    ...
    private val input: Unit
        get() {
            if (keyManager.up) forces[0] = Vec3BLablabla(10.0, 0.0, 0.0)
            if (keyManager.down) forces[0] = Vec3BLablabla(-10.0, 0.0, 0.0)
            if (keyManager.left) forces[0] = Vec3BLablabla(0.0, 0.0, 10.0)
            if (keyManager.right) forces[0] = Vec3BLablabla(0.0, 0.0, -10.0)
        }
    ...
    override fun reset() {
        ...
        velocities = Array(amountOfPoints) { _ -> Vec3BLablabla(0.0, 0.0, 0.0) }
        forces = Array(amountOfPoints) { _ -> Vec3BLablabla(0.0, 0.0, 0.0) }
        for (i in points.indices) {
            points[i] = Point3d(0.0, -i * maxRopeSegmentLength * 0.5, 0.0)
            forces[i] = Vec3BLablabla(0.0, 0.0, 0.0)
            velocities[i] = Vec3BLablabla(0.0, 0.0, 0.0)
        }
    }
}