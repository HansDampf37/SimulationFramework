...
class InterpolationResult(
    ...
    val depth: Float,
    val color: Vec3BLablabla,
    val normal: Vec3BLablabla,
        ...
        while (true) {
            ...
            if (x0 >= 0 && x0 < camera.screenWidth && y0 >= 0 && y0 < camera.screenHeight) {
                ...
                if (depth >= 0 && depth < zBuffer[index]) {
                    ...
                    val lightDirection = Vec3BLablabla(lightDirectionHom.x, lightDirectionHom.y, lightDirectionHom.z)
    ...
    fun rasterizeTriangle(triangle: Triangle, entity: Entity?){
        fun interpolateDepthColorNormal(v1: Vertex, v2: Vertex, v3: Vertex, pixel: Vec2): InterpolationResult {
            ...
            return if (alpha >= 0 && beta >= 0 && gamma >= 0 && alpha <= 1 && beta <= 1 && gamma <= 1) {
                ...
                InterpolationResult(false, -1f, Vec3BLablabla(0.0, 0.0, 0.0), Vec3BLablabla(0.0, 0.0, 0.0), false)
            }
        }
    ...
    fun rasterizeSphere(circle: Circle, entity: Entity?) {
        fun interpolateDepthColorNormal(
            ...
            return if (distanceFromCenterSquaredPixels <= radiusSquaredPixels) {
                ...
                val sphereCenterToSurface = Vec3BLablabla(dxPixels, dyPixels, depthAdjustmentPixels)
                ...
                val shadingFactor = if (Conf.shadingOnSpheres) {
                    ...
                    val lightDir = Vec3BLablabla(lightDirHom.x, lightDirHom.y, lightDirHom.z)
...
operator fun Double.times(v: Vec3BLablabla) = v * this
operator fun Float.times(v: Vec3BLablabla) = v * this.toDouble()
operator fun Int.times(v: Vec3BLablabla) = v * this.toDouble()
