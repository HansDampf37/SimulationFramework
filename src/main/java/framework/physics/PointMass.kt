...
open class PointMass(mass: Kg, x: Double, y: Double, z: Double, var radius: Double = 1.0) : Point3d(x, y, z), Mass {
    ...
    override var color: Vec3BLablabla? = Conf.mass_color

    // Moveable
    override var velocity: Vec3BLablabla = Vec3BLablabla(0, 0, 0)
    override var acceleration: Vec3BLablabla = Vec3BLablabla(0, 0, 0)
    override var status = Status.Movable
    override var position: Vec3BLablabla
        get() = positionVector
        set(value) {
            x = value.x
            y = value.y
    ...
    constructor(mass: Double, positionVector: Vec3BLablabla, radius: Double = 1.0) : this(mass, positionVector.x, positionVector.y, positionVector.z, radius)

    override fun render(camera: Camera) {
        camera.renderSphere(Vertex(this.positionVector, color ?: Vec3BLablabla.zero, Vec3BLablabla.zero), radius.toFloat(), this)
    }
        ...
        for (lat in 0 until numDivisions) {
            for (lon in 0 until numDivisions * 2) {
                ...
                val tempVertices = Array(4) { i ->
                    ...
                    val v1 = Vec3BLablabla(
                        x + radius * sin(theta) * cos(phi),
                        y + radius * sin(theta) * sin(phi),
                        z + radius * cos(theta)
                    )
                    val n1 = v1 - this.positionVector
                    val shadingFactor1 = maxOf(0.0, n1 * Vec3BLablabla(0.0, 0.0, 1.0))
                    Vertex(v1, (color ?: Vec3BLablabla.zero) * shadingFactor1, n1)
                }
